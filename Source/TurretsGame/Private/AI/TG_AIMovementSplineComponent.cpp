...
{
    if (MovementSpline.IsNull()) return FAISystem::InvalidLocation;
...
{
    if (MovementSpline.IsNull()) return;
    IndexMovementSpline += 1;
...
{
    if( !bCallMove || !AIController || !AIController->GetPawn()) return;
